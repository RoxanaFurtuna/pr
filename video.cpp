...
//green = {35,86,140,256,55,256}; 
//yellow= {59,67,76,152,0,256};
...
int r=40;
int out_of_ring(int x,int y,int xb,int yb){
	if(((red_x - xb)^2 + (red_y - yb)^2) < (r*r)) //e in aria cercului => nu a iesit afara
		return 1;
	return 0;
}
int main(int argc, char* argv[])
{
	...
	int x = 0, y = 0,xl=0,yl=0,xy=0,yy=0;
...
inRange(HSV, Scalar(H_MIN_BL, S_MIN_BL, V_MIN_BL), Scalar(H_MAX_BL, S_MAX_BL, V_MAX_BL), threshold);
		if (useMorphOps)
			morphOps(threshold);
		if (trackObjects)
			trackFilteredObject(xl, yl, threshold, cameraFeed);
    blk_x=xl;
    blk_y=yl;
//for yellow => for orientation
inRange(HSV, Scalar(H_MIN_Y, S_MIN_Y, V_MIN_Y), Scalar(H_MAX_Y, S_MAX_Y, V_MAX_Y), threshold);
    ...
    yel_y=yy;
		...
		if(x!=red_x){
			bzero(buffer,256);
			see_enemy(yel_x,yel_y,red_x,red_y,blue_x,blue_y);
			if(out_of_ring(red_x,red_y,blk_x,blk_y)==1)
				//sprintf(buffer, buff);
				strcpy(buffer,buff);
			else
